...
// set log output to a file (name or descriptor)
void log_output_file(int fd, uint64_t rotate_limit = UINT64_MAX);
int  log_output_file(const char* fn, uint64_t rotate_limit = UINT64_MAX, int max_log_files = 10);
int  get_log_file_fd();

// close output file, only after log output has been set to other destination
...
// handler function of log output
...
template<typename T, ENABLE_IF_NOT_SAME(T, char*)> static inline
...
template<typename T, ENABLE_IF_SAME(T, char)> static inline
...
static void __do_log__(const Prologue& prolog, const Ts&...xs)
{
    LogBuffer log;
    log << prolog;
    log.printf(xs...);
...
static inline void __log__(const Prologue& prolog, Ts&&...xs)
{
    __do_log__(prolog, alog_forwarding(xs)...);
}

template<typename SEQ=int, size_t N, typename...Ts>
static inline void __log__(const Prologue& prolog, char (&s)[N], Ts&&...xs)
{
    __do_log__(prolog, alog_forwarding(s), alog_forwarding(xs)...);
}

template<typename SEQ, size_t N, typename...Ts> __attribute__((noinline))
static void __log__(const Prologue& prolog, const char (&fmt)[N], Ts&&...xs)
{
    alog_format::FMTLogBuffer log;
    log << prolog;
...
#define __LOG__(level, first, ...)                          \
    if ((level) >= log_output_level) {                      \
        ...
        __log__<sequence>(prolog, first, __VA_ARGS__);      \
    }

struct alog_noop { };
static inline LogBuffer& operator << (LogBuffer& log, const alog_noop&) { return log; }
...
static inline void set_log_output_level(int l)
{
    log_output_level = l;
}
...
static inline LogBuffer& operator << (LogBuffer& log, ERRNO e)
{
...
static inline NamedValue<T> make_named_value(const char (&name)[N], T&& value)
{
...
template<typename T>
static inline LogBuffer& operator << (LogBuffer& log, const NamedValue<T>& v)
{
    return log.printf('[', v.name, '=', v.value, ']');
}

static inline LogBuffer& operator << (LogBuffer& log, const NamedValue<ALogString>& v)
{
    return log.printf('[', v.name, '=', '"', v.value, '"', ']');
}
