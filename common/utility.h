...
static inline uint64_t alingn_down(uint64_t x, uint64_t alignment)
{
    return x & ~(alignment - 1);
}

static inline uint64_t alingn_up(uint64_t x, uint64_t alignment)
{
    return alingn_down(x + alignment - 1, alignment);
}

template<typename T>
static inline T* align_ptr(T* p, uint64_t alignment)
{
